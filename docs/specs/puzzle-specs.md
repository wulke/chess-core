...
| PZL-007 | WHEN a puzzle-review session is created THE SYSTEM SHALL recover puzzle provenance through the root `PositionOccurrence` rather than requiring a direct `puzzle_id` field on `AnalysisSession` in v1. | [x] → #10 |
